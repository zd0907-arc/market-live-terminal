...
**v2.0 Update**: 后端全面升级为异步非阻塞架构，前端工程结构已标准化。

## 🏛 技术架构 (Architecture)

### 1. 前端 (Frontend)
- **技术栈**: React 19, TypeScript, Tailwind CSS, Recharts。
- **职责**: 
  - 界面渲染与交互。
  - **实时数据**: 直接通过 JSONP 请求腾讯/东方财富接口 (Web -> External API)。
  - **历史数据**: 调用本地 Python 后端接口 (Web -> Localhost:8001)。
  - **核心算法**: 在前端计算“主力活跃度”、“买卖占比”等衍生指标。

### 2. 后端 (Backend)
- **技术栈**: Python 3, FastAPI, Uvicorn, HTTPX (Async)。
- **职责**: 
  - 作为数据代理网关 (Data Proxy)。
  - 绕过浏览器跨域限制 (CORS)。
  - 异步调用外部财经接口，获取包含 **买入/卖出分离** 的资金流向数据。
...
2. 启动开发服务器：
   ...
   npm run dev
   ```
3. 打开浏览器访问 `http://localhost:3000` (或 3001)。
