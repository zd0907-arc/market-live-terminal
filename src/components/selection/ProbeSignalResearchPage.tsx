...
import { getProbeSignalResearchPage, PROBE_SIGNAL_RESEARCH_PAGES } from './probeSignalResearchRegistry';

const ProbeSignalResearchPage: React.FC = () => {
  const page = getProbeSignalResearchPage(typeof window === 'undefined' ? '' : window.location.pathname);
  ...
  if (!page) {
    return <div className="min-h-screen bg-[#0a0f1c] p-6 text-red-200">未识别的试盘研究页路由。</div>;
  }
    ...
    <div className="min-h-screen bg-[#0a0f1c] text-slate-200">
      <div className="sticky top-0 z-40 border-b border-slate-800 bg-[#0f1623]/95 shadow-md backdrop-blur">
        <div className="mx-auto flex max-w-[1800px] flex-wrap items-center gap-2 px-4 py-3 md:px-6">
          <a href="/model-training" className="inline-flex h-9 items-center gap-1 rounded-lg border border-slate-700 bg-slate-900 px-3 text-xs font-medium text-slate-200 hover:border-slate-500">
        ...
        <div className="mx-auto max-w-[1800px] px-4 pb-3 md:px-6">
          <div className="inline-flex max-w-full flex-wrap gap-2 rounded-xl border border-slate-800 bg-slate-950 px-2 py-2">
            {PROBE_SIGNAL_RESEARCH_PAGES.filter((item) => item.enabled).map((item) => {
      ...
      <main className="mx-auto max-w-[1800px] space-y-4 px-4 py-4 md:px-6">
        <SectionCard title="页面说明" icon={<BarChart3 className="h-4 w-4 text-fuchsia-300" />}>
          <div className="grid gap-3 md:grid-cols-5">
            <Metric label="当前分组" value={activeSection?.title || '--'} />
            <Metric label="当前样本" value={`${activeSection?.stock_count || 0} 只`} />
            <Metric label="原始信号" value={`${activeSection?.source_signal_count || 0} 次`} />
            <Metric label="平均10日冲高" value={fmtPct(avgRunup)} tone="text-red-200" />
            <Metric label="平均10日收盘" value={fmtPct(avgCloseReturn)} tone={pctTone(avgCloseReturn)} />
          </div>
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-md px-3 py-2 text-left text-sm transition ${active ? 'bg-fuchsia-500/15 text-fuchsia-100' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'}`}
                >
          ...
          <section className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-2">
              ...
              <div className="text-xs text-slate-400">
                样本 {activeSection.stock_count} 只 · 信号 {activeSection.source_signal_count} 次
              </div>
            </div>
            <div className="grid gap-4 xl:grid-cols-2">
              {activeSection.items.map((item) => <ProbePatternCard key={item.id} item={item} />)}
            </div>
          </section>
        ) : null}
      </main>
...
export default ProbeSignalResearchPage;
