...
interface FundsBattleL1PanelProps {
...
const EmptyState: React.FC<{ text: string; isLoading?: boolean }> = ({ text, isLoading }) => (
  <div className="min-h-[220px] flex items-center justify-center text-sm text-slate-500">
    {isLoading ? '正在加载资金博弈...' : text}
  </div>
);

const SideSummary: React.FC<{ points: BattlePoint[]; modeLabel: string }> = ({ points, modeLabel }) => {
  ...
  return (
    <div className="w-[60px] bg-slate-900/80 rounded-lg border border-slate-700/50 flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center border-b border-slate-700/30 relative overflow-hidden">
        <div className="text-[9px] text-slate-500 absolute top-1 z-10">{modeLabel}</div>
...
export const FundsBattleL1Panel: React.FC<FundsBattleL1PanelProps> = ({ data, isLoading = false, tuning = DEFAULT_FUNDS_BATTLE_TUNING }) => {
  const bars = data?.bars ?? [];
  const enableSignals = Boolean(data?.is_l2_finalized && data?.source !== 'history_l1_fallback');
  const { points } = useMemo(
    () =>
      buildBattleSeries(bars, 'l1', tuning, {
        enableSignals,
      }),
  ...
  if (isLoading) return <EmptyState text="正在加载资金博弈..." isLoading />;
  if (!data || points.length === 0) return <EmptyState text="当前暂无资金博弈数据" />;

  const title = data.mode === 'intraday_l1_only' ? '[ 🕒 盘中 L1 快照估算 ]' : '[ L1 表象资金博弈 (快照推演) ]';
  const modeLabel = data.mode === 'intraday_l1_only' ? '实时' : 'L1';

  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-2">
      <div className="mb-2 px-1 flex items-center gap-1.5 group relative">
        ...
        <div className="absolute left-0 bottom-full mb-2 w-64 p-3 bg-slate-950 border border-slate-700 rounded-lg shadow-xl text-[11px] leading-relaxed text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
          <div className="font-bold text-white mb-1">L1 / L2 对照标签</div>
          <div className="mb-1"><span className="text-red-400">吃 / 诱空</span>：基于 L2 真相回看 L1 表象的多头提示。</div>
          <div><span className="text-green-400">出 / 诱多</span>：基于 L2 真相回看 L1 表象的空头提示。</div>
        </div>
      </div>
      ...
      <div className="flex gap-1">
          ...
          <div className="bg-slate-900/50 rounded-lg p-2 border border-slate-800/50 relative">
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={points} syncId="fundsBattle5m" margin={{ top: 4, right: 4, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id={`funds-battle-l1-${data.symbol}`} x1="0" y1="0" x2="0" y2="1">
                ...
                <Area yAxisId="left" type="monotone" dataKey="cvd" stroke="none" fill={`url(#funds-battle-l1-${data.symbol})`} isAnimationActive={false} />
            ...
            <ResponsiveContainer width="100%" height={150}>
              <BarChart data={points} syncId="fundsBattle5m" margin={{ top: 4, right: 4, left: 10, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ backgroundColor: '#020617', borderColor: '#334155' }}
                  itemStyle={{ fontSize: 12 }}
                  formatter={(_: number, __: string, item: any) => [formatFundsYAxis(item?.payload?.oibReal), 'L1 OIB']}
                />
        ...
        <SideSummary points={points} modeLabel={modeLabel} />
      </div>
    </div>
  );
};

export default FundsBattleL1Panel;
