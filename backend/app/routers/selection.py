...
def selection_candidates(
    ...
    strategy: str = Query("breakout", description="stealth / breakout / distribution / v2"),
    limit: int = Query(10, ge=1, le=500),
    replay_validation: bool = Query(False, description="仅 v2 实验验证使用：按 Layer3 回放结果排序"),
):
    try:
        if str(strategy).lower() == "v2":
...
def selection_profile(
    symbol: str,
    date: str = Query(None, description="交易日 YYYY-MM-DD，缺省为最新可用日"),
    strategy: str = Query("breakout", description="breakout / stealth / distribution / v2"),
):
    try:
        if str(strategy).lower() == "v2":
