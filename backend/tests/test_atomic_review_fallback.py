...
def _reload_runtime_modules(monkeypatch, tmp_path):
    ...
    import backend.app.routers.review as review

    importlib.reload(config)
    importlib.reload(l2_history_db)
    importlib.reload(review)
    return config, l2_history_db, review
        ...
        conn.executemany(
            ...
            [
                ...
                (
                    ...
                    4, 95_000.0, 67_000.0, "unit-test", None,
                ),
            ],
        )
        conn.execute(
...
def test_query_daily_rows_falls_back_to_atomic(monkeypatch, tmp_path):
    _init_atomic_db(tmp_path)
    _, l2_history_db, _ = _reload_runtime_modules(monkeypatch, tmp_path)

    rows = l2_history_db.query_l2_history_daily_rows("sh603629", start_date="2026-04-09", end_date="2026-04-10")

    assert len(rows) == 2
    assert rows[-1]["date"] == "2026-04-10"
    assert rows[-1]["l2_main_net"] == 410_000.0
    assert rows[-1]["l2_super_ratio"] > 0


def test_review_router_uses_atomic_fallback(monkeypatch, tmp_path):
    _init_atomic_db(tmp_path)
    _, l2_history_db, review = _reload_runtime_modules(monkeypatch, tmp_path)

    l2_history_db.replace_stock_universe_meta(
        [("sh603629", "利通电子", 8_800_000_000.0)],
        as_of_date="2026-04-10",
        source="unit-test",
    )

    pool_resp = review.get_review_stock_pool("", 10)
    assert pool_resp.code == 200
    assert pool_resp.data["latest_date"] == "2026-04-10"
    assert pool_resp.data["items"][0]["symbol"] == "sh603629"
    assert pool_resp.data["items"][0]["name"] == "利通电子"

    intraday_resp = review.get_review_data("sh603629", "2026-04-10", "2026-04-10", "15m")
    assert intraday_resp.code == 200
    assert len(intraday_resp.data) == 1
    assert intraday_resp.data[0]["bucket_granularity"] == "15m"
    assert intraday_resp.data[0]["l2_main_net"] == 410_000.0
    assert intraday_resp.data[0]["source_date"] == "2026-04-10"

    daily_resp = review.get_review_data("sh603629", "2026-04-10", "2026-04-10", "1d")
    assert daily_resp.code == 200
    assert len(daily_resp.data) == 1
    assert daily_resp.data[0]["bucket_granularity"] == "1d"
    assert daily_resp.data[0]["l2_super_net"] == 190_000.0
