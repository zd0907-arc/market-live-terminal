...
from backend.scripts.analyze_strategy_theme_lifecycle import build_rank_history, classify_stage
...
def add_return(bucket: Dict[str, Dict[str, Dict[str, List[float]]]], strategy: str, hkey: str, policy: str, ret: float) -> None:
    bucket[strategy][hkey][policy].append(ret)


def render_markdown(report: Dict[str, Any]) -> str:
    meta = report["meta"]
    lines = [
        f"# 热点生命周期排雷策略验证 {meta['start_date']} ~ {meta['end_date']}",
        "",
        f"- 候选：每日每策略 Top{meta['selection_top_k']}。",
        f"- 热点：细颗粒 Top{meta['hot_top_k']} + 日内共振。",
        f"- climax_hot：过去 {meta['climax_lookback']} 日至少 {meta['climax_min_hits']} 日进入 Top{meta['climax_top_k']}。",
        "",
    ]
    for strategy, by_h in report["summary"].items():
        lines += [f"## {strategy}", "", "| Horizon | 策略口径 | 样本 | 保留率 | 均值 | Alpha | 胜率 | 相对Baseline均值 |", "|---:|---|---:|---:|---:|---:|---:|---:|"]
        for hkey, policies in by_h.items():
            base_n = policies.get("baseline", {}).get("n") or 1
            base_avg = policies.get("baseline", {}).get("avg") or 0
            for policy, stat in policies.items():
                lines.append(
                    f"| {hkey} | {policy} | {stat['n']} | {stat['n']/base_n:.1%} | {stat['avg']:.2f}% | {stat['alpha']:.2f}% | {stat['win_rate']:.1%} | {stat['avg'] - base_avg:+.2f}% |"
                )
        lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate whether hot lifecycle tags can be used as risk-exclusion policy.")
    ...
    themes, _theme_members, symbol_themes, name_map = load_fine_sector_themes(
        Path(args.tradable_theme_db), rules, args.min_member_count, args.max_member_count
    )
    ...
    for rec in candidates:
        ...
        if i is None or not rows or i + 1 >= len(trade_dates):
            continue
        ...
        for horizon in horizons:
            ...
            # Strategy-specific policy buckets.
            add_return(returns, strategy, hkey, "baseline", ret)
            if stage != "climax_hot":
                add_return(returns, strategy, hkey, "exclude_climax", ret)
            if stage not in {"new_hot", "climax_hot"}:
                add_return(returns, strategy, hkey, "exclude_new_and_climax", ret)
            if stage == "non_resonance":
                add_return(returns, strategy, hkey, "non_resonance_only", ret)
            if strategy == "stealth" and stage == "new_hot":
                add_return(returns, strategy, hkey, "new_hot_only", ret)

            # Combined portfolio-like buckets: only act on breakout risk tags; stealth is not excluded.
            add_return(returns, "all", hkey, "baseline", ret)
            if not (strategy == "breakout" and stage == "climax_hot"):
                add_return(returns, "all", hkey, "exclude_breakout_climax", ret)
            if not (strategy == "breakout" and stage in {"new_hot", "climax_hot"}):
                add_return(returns, "all", hkey, "exclude_breakout_new_and_climax", ret)
            if not (strategy == "breakout" and stage != "non_resonance"):
                add_return(returns, "all", hkey, "exclude_breakout_any_resonance", ret)

    summary: Dict[str, Dict[str, Any]] = {}
    policy_order = {
        "breakout": ["baseline", "exclude_climax", "exclude_new_and_climax", "non_resonance_only"],
        "stealth": ["baseline", "exclude_climax", "exclude_new_and_climax", "non_resonance_only", "new_hot_only"],
        "all": ["baseline", "exclude_breakout_climax", "exclude_breakout_new_and_climax", "exclude_breakout_any_resonance"],
    }
    for strategy, by_h in returns.items():
        summary[strategy] = {}
        for hkey, policies in by_h.items():
            summary[strategy][hkey] = {}
            for policy in policy_order.get(strategy, sorted(policies.keys())):
                stat = summarize(policies.get(policy, []))
                stat["alpha"] = round(stat["avg"] - market[hkey]["avg"], 4)
                summary[strategy][hkey][policy] = stat
    ...
    out_path = Path(args.output) if args.output else MARKET_HEAT_DIR / (
        f"strategy_theme_risk_policy_top{args.selection_top_k}_hot{args.hot_top_k}_"
