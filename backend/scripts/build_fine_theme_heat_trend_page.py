...
OUT = ROOT / 'docs/selection/market_heat/fine_theme_heat_trend_2026_v2.html'
START = '2026-01-05'
END = '2026-04-30'
WARMUP = '2025-10-01'
...
# 仍用已回补热点表决定“今年最值得看的30个标签”；但后续走势全部重算为连续序列。
top = hc.execute(
    '''
    with s as (
    ...
    select * from s
    order by top15_days desc, best_rank asc, max_hot desc
    limit 30
    ''',
    (START, END),
).fetchall()
...
svg{{width:100%;height:auto;display:block;background:#0d1324;border-radius:9px}} .gridline{{stroke:#25314c;stroke-width:1}} .month{{stroke:#33415f;stroke-width:1;stroke-dasharray:4 4}} .axis{{fill:#8192b0;font-size:10px}} .price{{fill:none;stroke-width:2.0;opacity:.38}} .ma5{{fill:none;stroke:#ffffff;stroke-width:2.2;opacity:.88}} .ma10{{fill:none;stroke:#cbd5e1;stroke-width:1.8;opacity:.62;stroke-dasharray:5 4}} .heat{{fill:none;stroke:#facc15;stroke-width:1.8}} .heatfill{{fill:#facc15;opacity:.12}} .bar{{fill:#38bdf8;opacity:.38}} .dot{{stroke:#0d1324;stroke-width:1}}
...
</style></head><body><div class="wrap"><h1>2026 年小颗粒热点：连续热度 × 板块价格指数</h1>
<div class="sub">修正版：热度不再使用 Top50 截断值，而是对 Top30 主题逐日重算连续热度；价格指数为成员等权复合指数，起点=100，每个交易日都有。</div>
...
<div class="card"><div class="toolbar"><button id="top10" class="active">Top10</button><button id="all30">Top30</button><button id="clear">清空</button></div></div>
