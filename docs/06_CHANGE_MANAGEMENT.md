...
> 标准开发动作清单见：`docs/ops/development-workflow.md`
...
当任务进入治理 / 收口 / 复盘时，优先补齐这四类产物：

1. **代码审查发现**：形成独立 review findings，小结共享壳、双轨、边界、回归风险；
2. **项目健康快照**：形成 health snapshot，记录 branch / worktree / version / pending / 风险；
3. **主题母卡**：形成一张 `MOD-* current-state` 母卡，作为该主题的当前真相入口；
4. **归档摘要**：当主题结束后，把旧过程卡压成 archive summary，不再让过程卡充当默认入口。

治理类任务的默认顺序是：`review -> health -> mother card -> archive`。
