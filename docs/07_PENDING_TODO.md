...
- 当前事实：
  ...
  - 若要把更多历史月份补进新版历史多维，最稳妥方案不是“全市场全历史一次性猛跑”，而是**固定池快照 + 逐月往前跑 + 每月单独上线**。
- 目标：
  1. 冻结一份用于历史多维扩展的固定股票池快照；
  2. 从 `2026-02 -> 2026-01 -> 2025-12 ...` 逐月往前提升；
  3. 每个月完成后做月度验收，并立即在生产库可用。
- 解除条件：
  1. 固定池快照已导出并登记；
  2. 至少完成首个月份（建议 `2026-02`）的正式提升、质量汇总与生产验收；
  3. 已形成“月度提升 + 月度验收 + 继续下一月”的固定模板。
