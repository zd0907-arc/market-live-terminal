...
- 当前事实：
  ...
  - 同日已继续新增《原子事实层字段差异执行表》，把目标字段正式分成三类：
    - **已有，可直接映射**
    - **可算，不必回 raw**
    - **必补，必须新增清洗产出**
  - 当前已经可以明确先做的 P0 顺序：
...
- 当前事实：
  ...
  - 也已冻结原则：**集合竞价必须与连续竞价隔离**，不能默认把 `09:25` 并入 `09:30` 第一根连续竞价 bar；
  - 但当前还没有拍板“最终怎么落库”；
  - 同日已新增审计脚本 `backend/scripts/audit_l2_auction_window.py` 与方案文档 `STG-20260411-10-auction-window-audit-plan.md`，用于先查清 raw 在 `09:15~09:25` 的真实覆盖情况；
  - 第一轮 Windows 样本已跑出：
...
- 目标：
  ...
  3. 基于样本先冻结一版落库草案 V1；
  4. 后续再决定：是否单独落 `09:25` bar、是否加 `session_phase`、是否纳入收盘竞价。
- 关联任务：`CHG-20260411-12`

## T-026 原子事实层生产切换与正式发版
- 状态：`ACTIVE`
- 当前事实：
  - 当前生产冻结基线已有正式锚点：`v4.2.32 / 9bbdd3d`；
  - Windows 正式 atomic 主板库已回补完成；
  - 本地主路径已基本完成 atomic 接入：复盘 / 历史多维 / 选股 research；
  - 当前开发版停留在 `codex/atomic-integration-migration`，尚未 merge 到 `main`。
- 下一步：
  1. 继续把剩余页面/接口收口为 atomic 主路径；
  2. 合回 `main`；
  3. 正式版本归一为候选 release（建议 `v4.3.0`）；
...
- 状态：`ACTIVE`
- 当前事实：
  - 截至 `2026-04-19`，后端已新增独立官方事件层：`stock_events / stock_event_entities / stock_event_ingest_runs / stock_event_daily_rollup`；
  - 已新增 `backend/app/services/stock_events.py`、`/api/stock_events/feed/{symbol}`、公告同步入口、深市互动问答同步入口、沪市互动问答同步入口、财经快讯同步入口、长篇资讯同步入口，以及 watchlist 批量/单票回补入口；
  - 第一阶段已接通三类事实源：`公告 / 财报`、`上市公司互动问答`、`财经资讯`；
  - watchlist 新增股票时，会 best-effort 触发最近 `365D` 公告回补 + 最近 `180D` 互动问答回补 + 最近 `30D` 财经资讯回补；
  - 当前财经资讯已能进入官方事件层，且已升级到“代码 + 名称 + 简称裁剪”的多证据单票归属，并已把关联方法/置信度/关联股票写入元数据；
  - 选股画像时间线已开始合并读取 `stock_events`，不再只看股吧与情绪日评分。
  - 在**无 `TUSHARE_TOKEN`** 场景下，公告/业绩预告现已支持新浪公开页 fallback，且已完成三只样本票实跑：`利通电子 193` 条、`中百集团 91` 条、`贝因美 129` 条（最近 `365D`）。
- 下一步：
  1. 继续补**无 Token**链路下的 `QA / media news` 公开源；
  2. 把当前 `seed_file` alias 机制扩到更多样本票，逐步沉淀正式曾用名 / 历史简称词典；
  3. 用 `GET /api/stock_events/audit/{symbol}` 持续抽检真实环境单票采集完整性，确认公告 / 问答 / 资讯覆盖质量；
  4. 若后续接回 `TUSHARE_TOKEN`，再把官方付费链路与公开 fallback 做优先级编排，而不是互相覆盖；
  5. 视需要补板块/主题层事件，并评估是否把 `stock_event_daily_rollup` 并入选股事件计数口径。
