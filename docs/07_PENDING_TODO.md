...
- 状态：`ACTIVE`
- 当前事实：
  - Windows 正在继续下载 `2026-03` 盘后 L2 日包；
  - 基础适配与单日回补链路已就绪，但全月数据尚未完成“下载 → 清洗 → 正式入库 → 查询抽样”闭环。
- 目标：
...
- 解除条件：
  ...
  3. 缺包、坏包、字段异常有失败清单可追溯。
- 关联任务：`CHG-20260314-08`

## T-011 盯盘页新版信息架构收敛（当日分时 + 历史多维）
- 状态：`TODO`
- 当前事实：
  - 当前新版只完成了“整页切换 + 日线融合版 V1”；
  - 你最新确认的新范围是：新版只保留两个区域——`当日分时` 与 `历史多维`；
  - 历史多维需统一支持 `5m / 30m / 1h / 日` 切换，不再把 `30分钟线` 与 `日线` 分成两个独立 Tab。
- 目标：
