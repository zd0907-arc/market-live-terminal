...
- 目标：
  1. 在“外部控制端 SSH 编排”与“Windows 本机 OS 级控制器”之间二选一并固化；
  2. 若选择 Windows 本机自动化，必须产出正式控制脚本/任务计划定义，而不是继续使用当前 `l2_day_sharded_backfill.py`；
...
- 当前事实：
  ...
  - 若要把更多历史月份补进新版历史多维，最稳妥方案不是“全市场全历史一次性猛跑”，而是**固定池快照 + 逐月往前跑 + 每月单独同步生产**。
- 目标：
  1. 冻结一份用于历史多维扩展的固定股票池快照；
  2. 从 `2026-02 -> 2026-01 -> 2025-12 ...` 逐月往前回补；
  3. 每个月完成后做月度验收，再增量同步到生产，让新版历史多维立即可用。
- 解除条件：
  1. 固定池快照已导出并登记；
  2. 至少完成首个月份（建议 `2026-02`）的正式回补、质量汇总与生产同步；
  3. 已形成“月度 manifest + 月度验收 + 月度同步”的固定模板。
