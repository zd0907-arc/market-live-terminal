...
- `代码提交、小配置、小模型文件、单个小数据补丁` 可以走 `dxp4800pro` / Tailscale。
- `research/current` 整包、数 GB 级数据库、批量报告资产、大目录同步，只在家里局域网执行，优先使用 `192.168.3.43`。
- 如果当前不在局域网，不继续硬传大文件；只记录待同步清单，等回到局域网后再跑。
- 局域网同步前先做一次小文件测速或 `rsync --dry-run` / `tar` 清单检查，确认路径和吞吐正常后再传正式包。
...
- `外网 / Tailscale 环境`：只做代码、配置、小文件、远程查数和服务验证。
- `家里局域网环境`：再做 `research/current`、大数据库、批量文档和模型产物同步。
- 如果本轮问题只卡在大容量同步上，默认不改架构、不改主链，标记为“回家局域网后执行”。
