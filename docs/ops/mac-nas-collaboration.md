...
- Tailscale 控制面显示 `dxp4800pro` 在线
- `tailscale ping dxp4800pro` 可以通
- 但 `SSH / HTTP / Gitea SSH` 这几条 TCP 业务链路本次复检出现超时

所以当前准确判断是：

- `Mac -> NAS` 这套工作模式是对的，前面也已经实测跑通过
- 但今天需要把 `TCP 服务连通性` 当成单独故障来排，不要误判成方案本身错误
...
下表区分“曾经实测通过”和“本次复检状态”。

| 能力 | 曾实测通过 | 2026-06-04 本次复检 |
|---|---|---|
| Tailscale 节点在线 | 是 | 通过 |
| `tailscale ping dxp4800pro` | 是 | 通过 |
| SSH 登录 NAS | 是 | 本次超时 |
| 远程 `python3` / `sqlite3` | 是 | 依赖 SSH，本次未重跑 |
| NAS 目录创建 | 是 | 依赖 SSH，本次未重跑 |
| `scp -O` 上传文件 | 是 | 本次未重跑 |
| Gitea SSH 认证 | 是 | 本次超时 |
| `git ls-remote nas` | 是 | 本次未复检成功 |
| 项目服务私网访问 | 是 | 本次超时 |
| 项目服务临时公网访问 | 是 | 本次超时 |
...
这说明下一步优先级不是重做方案，而是排查 NAS 当前的 Tailscale / Funnel / Docker 端口暴露状态。
