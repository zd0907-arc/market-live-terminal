...
## 4. 当前总判断
1. 当前主线是：**Windows 数据主站 + Mac 本地研究站 + NAS 在线运行节点**。
2. 当前最重要的长期建设方向：
   - 原子层与旧依赖继续收口
   - 选股研究继续验证策略有效性与真实覆盖面
...
5. 每日盘后正式跑数入口是 `ops/run_daily_new_framework.sh --json --sync-nas`；默认不传日期，由脚本自动检测 Windows 日包与 Mac 本地完整性，并把市场环境指数、热点结果、热点页面缓存、选股工作台的模型/策略跑数、本地必需模型产物和 NAS 同步一起纳入完成标准。
