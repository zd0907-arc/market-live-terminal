...
> **【20GB 云盘生命线防爆机制】**：尽管理论容量足够，但因腾讯云服务器总硬盘仅有 20GB（须刨去系统占用），为防止异常暴涨导致宕机，我们确立以下**强制削峰规则**：
> 1. 云端 `trade_ticks` 仅保留最近 **30 个交易日**的明细数据（用于支持“近一个月任意一天的分时图回看”）。
> 2. 超过 30 天的原始 Tick 将通过定时脚本无情抹除。因为它的残值（主力净流入、超级大单）已经被完美固化沉淀到了 `local_history` 和 `history_30m` 中，原始 Tick 对我们的长线量化已无作用。
