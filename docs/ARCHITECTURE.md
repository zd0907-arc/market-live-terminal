...
### 2.1 开发与生产数据隔离边界 (Cloud-Local Data Boundary)
- **核心规约**: 本地开发环境的 Mac **绝不**用来存放需要长期持久化的生产数据。所有的真实清洗、打包下放等脏活累活交由独立的 Windows 节点（离线算力网关）或挂载大容量云盘的 Ubuntu 处理。
- **本地开发法宝**: 开发者在本地编写 UI 或测试新逻辑前，应执行根目录下的 `./sync_cloud_db.sh` 脚本。该脚本会将生产服务器上最新的 `market_data.db` 原封不动地 `scp` 覆盖到本地根目录。
- **目的**: 确保本地 `npm run dev` 能吃到 100% 真实的复杂数据边界，避免伪造测试数据掩盖真实 Bug，同时也根绝了双胞胎数据库引发的代码错乱。
