...
### 2.1 云地协同架构边界 (Cloud-Local Data Boundary) - 必读铁律！
*   **致命陷阱（AI 历史血泪教训）**：绝不能认为 Windows/Mac 在局域网只负责“干一次历史回填”。由于云端服务器的公网 IP 被东方财富等金融接口**永久高亮风控（封IP）**，云端在交易时间内**完全丧失**实时拉取 `trade_ticks` 和快照的能力。
*   **核心规约 (The Rule)**：
    *   **云端 (Cloud/Ubuntu)** 是纯粹的【数据主库 + Web展示层】。它提供 24 小时随时访问的 UI 界面，以及安全、集中的 SQLite 存储。**云端绝对不主动运行爬虫**。
    *   **家庭算力网关 (Windows PC)** 是【历史回填引擎 + 盘中高频爬虫】的双料打工仔。在日常交易日中，Windows 会伪装成普通家庭宽带用户，高频轮询（3秒快照/3分钟明细）自选股的变化。
    *   **数据贯通管线 (The Bridge)**：Windows 爬取后，**不存本地**，而是实时通过调用云端 FastAPI 暴露的 `POST /api/internal/ingest` 接口（或类似隧道），将活水数据源源不断地泵入云端数据库，从而驱动网页端的 30 分钟 K 线图实时跳动。

### 2.2 前端 (表现层)
- **核心框架**: React 19, TypeScript, Vite.
- **UI 组件库**: Tailwind CSS, Recharts, Lucide-React.
- **主要职责**:
  - **RealtimeView (实时视图)**: 渲染实时交易逐笔和资金流向图表。在前端利用共享逻辑计算核心指标。
  - **HistoryView (历史视图)**: 可视化从后端获取的历史趋势数据（已加入 Error Boundaries 以处理云端未收集到的真空期数据）。
