...
### 0.5.1 价格口径
- 当前历史 OHLC 默认解释为：**真实价格 / 未复权价格（Raw Price）**。
- 后续若进入跨日波段研究、日级回测、均线/区间涨幅分析，必须结合**复权因子**读取，不得把除权缺口误当作真实波动。
- 复权因子不应污染原子成交事实表；建议未来以独立表或 Data Loader 动态处理。

### 0.5.2 5 分钟桶开闭区间
- 5m bar 统一约定为：**前闭后开** `[t, t+5m)`。
- 时间戳统一打左端点。
- 例如：
  - `09:30:00` 代表 `[09:30:00, 09:35:00)`
  - `14:30:00` 代表 `[14:30:00, 14:35:00)`
- 该约定必须在后续 1m/5m/日级对齐、回测与信号研究中保持一致，避免 look-ahead bias。

### 0.5.3 集合竞价特殊约定（待补充）
- 集合竞价与连续竞价不能默认混桶处理。
- 当前已冻结“**需要隔离**”这一原则，但**具体落库形态仍待下一轮专题讨论**，暂未最终确定：
  1. 是不是单独落 `09:25:00` bar；
  2. 是否增加 `session_phase`；
  3. 早盘竞价 raw 当前到底覆盖到什么粒度。
- 在方案未冻结前，后续新脚本不得擅自把 `09:25` 结果并入 `09:30` 第一根连续竞价 bar。
...
>
> **增强预留（尚未冻结到正式 schema）**：
> - 集合竞价需要与连续竞价分离；当前只冻结原则，不冻结具体字段；
> - 后续若进入原子事实层，建议增加 `session_phase` 或独立竞价 bar；
> - 若后续做涨跌停/触板研究，`5m` 层建议增加 `is_limit_up / is_limit_down / touch_limit_up / touch_limit_down` 这类状态增强字段。
...
> - 后续若进入波段回测/跨日研究，读取层必须明确区分 raw price 与 adjusted price。
> - 后续若进入涨跌停行为研究，日级层建议补 `limit_up_price / limit_down_price / hit_limit_up / close_at_limit_up / first_limit_up_ts` 等状态字段，但当前尚未冻结到正式生产 schema。
