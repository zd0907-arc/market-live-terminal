...
- 已将治理结果 fast-forward 合并到 `main` 并推送 GitHub。
- 已完成分支治理：当前仅保留 `main` + `codex/archive-pre-governance-20260318` 两个长期分支；历史混乱分支已删除。
- 对远端残留的历史中文命名分支，已先转存为归档 tag `archive-legacy-v2.8-local-final-20260318`，再删除原分支，避免未来继续污染主线分支视图。
