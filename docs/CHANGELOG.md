...
- **Local-to-Cloud Sync**: 针对云端 IP 被东财物理封锁的问题，全新引入 `sync_local_to_cloud.sh` 脚本。该脚本允许直接在本地网络环境绕过云端封锁抓取数据，并在本地组装为 SQL 包后，通过 SSH 加密隧道直接直连并贯通注入到云端 SQLite 数据库中，实现云管端物理隔离但数据无缝下发。
