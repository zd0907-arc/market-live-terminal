...
为了实现您说的“以后再也不想摸那台 Windows 电脑”，我们彻底抛弃手动点击，利用 Windows 自带的自启功能：

1. **唯一一次接触 Windows**（配置开机启动）：
   * 在 Windows 电脑上按 `Win + R`，输入 `shell:startup`，回车打开启动文件夹。
   * 把项目根目录下的 **`start_live_crawler.bat`** 文件，右键“发送到 -> 桌面快捷方式”。
   * 把桌面上那个快捷方式，剪切粘贴到刚刚打开的 `shell:startup` 启动文件夹里。
2. **以后永远无人值守**：
   * 只要这台机器一开机（断电重启也行），它就会在后台自动运行 `live_crawler_win.py` 这个守护神。
   * 在 `09:15 ~ 15:05` 的交易时段，它勤勉抓取发送；在一天的其余时间及周末，它会自动进入深度休眠（Idle），CPU 占用为 `0%`。
   * 您再也不用管它了。
