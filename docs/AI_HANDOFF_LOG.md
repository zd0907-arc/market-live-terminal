...
## 2026-03-18 23:40 | 发布 AI
- Task ID: `CHG-20260318-03`
- CAP: `CAP-SECURITY`, `CAP-RELEASE`
- 结论: 发布后冒烟发现生产 Nginx 对全部 `/api` 无差别注入 `X-Write-Token`，导致匿名公网请求也可写 watchlist；已改为移除生产代理自动注入、前端默认只读，并新增“当前浏览器会话管理员写令牌”入口与安全护栏测试，版本提升至 `v4.2.23`。
- 风险: 生产环境写操作不再“开箱即用”；若管理员未先在受信浏览器会话录入令牌，星标/配置/手动抓取会返回 401。
- 链接: `deploy/nginx.conf`, `src/config.ts`, `src/components/common/ConfigModal.tsx`, `backend/tests/test_security_guards.py`, `docs/changes/MOD-20260318-03-write-token-enforcement-hotfix.md`
