...
治理类任务默认产出以下四类文档产物，不把它们当成独立 skill：

- **Code review findings**：记录共享代码、跨页逻辑、数据链路的审查发现、严重度、修复顺序。
- **Health snapshot**：记录当前分支、worktree、版本、文档窗口、pending、数据链路健康度。
- **Current-state mother card**：记录某一主题“当前到底是什么”，作为后续所有过程卡的第一阅读入口。
- **Archive summary**：记录已结束主题的历史压缩结论，用于替代散落过程卡继续充当真相源。

这些产物都应回到 `docs/changes/`、`docs/archive/`、`docs/AI_HANDOFF_LOG.md` 的既定流程里，不在聊天记录里长期承载。
