...
if [ $# -lt 1 ]; then
  echo "usage: bash ops/nas/nas_publish_research_release.sh <release_name>" >&2
  exit 1
fi
...
ssh -o ConnectTimeout=8 "$NAS_HOST" "
  ...
  if [ -d \"\$current_dir\" ] && [ \"\$(find \"\$current_dir\" -mindepth 1 -maxdepth 1 | head -n 1)\" ]; then
    ...
    if [ -f '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' ]; then
      bash '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' \"\$archive_dir\" || true
    else
  ...
  if [ -f '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' ]; then
    bash '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' \"\$current_dir\"
  else
