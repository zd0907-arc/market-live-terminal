...
if [ $# -lt 1 ]; then
  echo "usage: bash ops/nas/upload_nas_research_release.sh <release_name>" >&2
  exit 1
fi
...
if [ ! -f "$MEMBERS_FILE" ] || [ ! -f "$MANIFEST_PATH" ]; then
  bash "$ROOT_DIR/ops/nas/build_nas_research_release_manifest.sh" "$RELEASE_NAME" >/dev/null
fi
...
ssh "${SSH_ARGS[@]}" "$NAS_HOST" "
  set -e
  if [ -f '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' ]; then
    bash '$NAS_APP_ROOT/ops/nas/rewrite_market_heat_release_metadata.sh' '$NAS_RELEASE_DIR'
  else
