...
REMOTE_SCRIPT="${WIN_ROOT}/backend/scripts/legacy/compat/build_local_research_snapshot.py"
...
ssh "$WIN_HOST" "cmd /c if not exist \"${WIN_ROOT}\\backend\\scripts\\legacy\\compat\" mkdir \"${WIN_ROOT}\\backend\\scripts\\legacy\\compat\" && if not exist \"${WIN_ROOT}\\data\\local_research\" mkdir \"${WIN_ROOT}\\data\\local_research\""

echo "[research-sync] upload snapshot builder"
scp "$ROOT/backend/scripts/legacy/compat/build_local_research_snapshot.py" "$WIN_HOST:${REMOTE_SCRIPT}"

if [ -z "$REMOTE_SELECTION_DB" ]; then
  REMOTE_SELECTION_DB="$(ssh "$WIN_HOST" "${WIN_PY_CMD} -c \"from pathlib import Path; p=Path(r'${WIN_ROOT_PY}/data/selection/selection_research.db'); print(str(p) if p.exists() else '')\"" | tr -d '\r' | tail -n 1)"
fi
...
REMOTE_CMD="cd /d ${WIN_ROOT} && ${WIN_PY_CMD} backend\\scripts\\legacy\\compat\\build_local_research_snapshot.py --output-db \"${REMOTE_OUTPUT_DB}\" --manifest-json \"${REMOTE_MANIFEST_JSON}\" --daily-days ${DAILY_DAYS} --intraday-days ${INTRADAY_DAYS} --sentiment-days ${SENTIMENT_DAYS} --signal-days ${SIGNAL_DAYS} --signal-limit ${SIGNAL_LIMIT}"
if [ -n "$REMOTE_SELECTION_DB" ]; then
