...
if [ -z "${DB_PATH+x}" ] \
  && [ -z "${USER_DB_PATH+x}" ] \
  && [ -z "${ATOMIC_MAINBOARD_DB_PATH+x}" ] \
  && [ -z "${ATOMIC_DB_PATH+x}" ] \
  && [ -z "${ATOMIC_DEFAULT+x}" ] \
  && [ -z "${1:-}" ] \
  && [ ! -d "$FORMAL_DATA_ROOT_DEFAULT" ]; then
  echo "[atomic-backend] 未找到 formal 数据根: $FORMAL_DATA_ROOT_DEFAULT" >&2
  echo "[atomic-backend] 请显式传入 DB_PATH/USER_DB_PATH/ATOMIC_MAINBOARD_DB_PATH，或先同步 formal market-data" >&2
  exit 1
fi

if [ -d "$FORMAL_LIVE_ROOT_DEFAULT" ]; then
  DB_ROOT_DEFAULT="$FORMAL_LIVE_ROOT_DEFAULT"
else
  DB_ROOT_DEFAULT="$FORMAL_DATA_ROOT_DEFAULT"
fi

DB_PATH_DEFAULT="$DB_ROOT_DEFAULT/market_data.db"
USER_DB_PATH_DEFAULT="$DB_ROOT_DEFAULT/user_data.db"
if [ -d "$RESEARCH_ROOT_DEFAULT" ]; then
  ATOMIC_ROOT_DEFAULT="$RESEARCH_ROOT_DEFAULT"
else
  ATOMIC_ROOT_DEFAULT="$FORMAL_DATA_ROOT_DEFAULT"
fi
ATOMIC_DEFAULT="${ATOMIC_DEFAULT:-$ATOMIC_ROOT_DEFAULT/atomic_facts/market_atomic_mainboard_compact_current.db}"
...
if [ ! -f "$DB_PATH" ]; then
  echo "[atomic-backend] 未找到 market DB: $DB_PATH" >&2
  exit 1
fi

if [ ! -f "$USER_DB_PATH" ]; then
  echo "[atomic-backend] 未找到 user DB: $USER_DB_PATH" >&2
  exit 1
fi
